...
def main():
    ...
    #possible with pro
    #import arcpy
    #token_obj = arcpy.GetSigninToken()
    #token = token_obj["token"]
   
    #portalUrl = "https://www.arcgis.com/sharing/rest/portals/self?f=json&token={}".format(token)
    #r = requests.get(portalUrl)
    #portalObject = r.json()
    #userName = portalObject["user"]["username"]

    #search service
    LogMessage("Searching for featureservice itemid")
    serviceID = GetAGOLIDByNameAndType("Windmills_EGT16","Feature Service",token)
    ...
    f = open(r"D:\EGT16\Part 3 - ArcGIS Services\webmapdata.json","r")
    webmapData = f.read()
    ...
    webmapInfo = {}
    webmapInfo["title"] = "Windmill Map EGT16"
    webmapInfo["tags"] = "Windmills_Tag"
    webmapInfo["description"] = "Hello Wereld, this is newly created webmap"
...
def GetLogger():
    global _logger
    if _logger == None:
        fh = logging.FileHandler(r"D:\EGT16\Part 3 - ArcGIS Services\Logs\10-createwebmap_{}.log".format(datetime.datetime.now().strftime('%Y%m%d_%H%M%S')))
