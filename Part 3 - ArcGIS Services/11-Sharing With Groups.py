...
def main():
    ...
    webMapID = GetAGOLIDByNameAndType("Windmill Map EGT16","Web Map",token)
    LogMessage("WebMap itemid: {}".format(webMapID))

    #get groupid
    wherestring='title:"Esri GISTech Python 2016" AND owner:"'+UserName+'"'
...
def GetLogger():
    global _logger
    if _logger == None:
        fh = logging.FileHandler(r"D:\EGT16\Part 3 - ArcGIS Services\Logs\11-sharewithgroups_{}.log".format(datetime.datetime.now().strftime('%Y%m%d_%H%M%S')))
